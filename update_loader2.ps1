...
if ($b64Target.Length % 4 -ne 0) { $b64Target += "=" * (4 - ($b64Target.Length % 4)) }
if ($b64Replacement.Length % 4 -ne 0) { $b64Replacement += "=" * (4 - ($b64Replacement.Length % 4)) }
...
if ($target) { $text = $text.Replace($target, $replacement) }
...
if ($b64Target2.Length % 4 -ne 0) { $b64Target2 += "=" * (4 - ($b64Target2.Length % 4)) }
if ($b64Replacement2.Length % 4 -ne 0) { $b64Replacement2 += "=" * (4 - ($b64Replacement2.Length % 4)) }
...
if ($target2) { $text = $text.Replace($target2, $replacement2) }
